# Неофициальный русский перевод книги "Crafting Interpreters"

## Инструкция для желающих помочь с переводом:

1. форкните репозиторий (ветка по умолчанию, **translation-ru**)
2. внесите любые минимальные изменения
3. откройте PR (из своего форка в ветку translation-ru TG), в названии указав "[WIP] Название-раздела-который-планируете-переводить”
4. в теле PR укажите список чекбоксов: по пунктам, что планируете переводить и оценку сроков (см. [пример](https://github.com/translation-gang/ru.crafting-interpreters/pull/2))
...
Перевод ведётся в ветке **translation-ru**, в ветке же **master** хранится оригинал. В репозитории поддерживается прямое соответствие версий оригинала и перевода. Для обновления оригинала открывается PR в ветку master, который остаётся открытым пока рядом не появится второй — в ветку translation-ru. Закрываются PR'ы одновременно, таким образом консистентность сохраняется.

## Ниже — оригинальное README
